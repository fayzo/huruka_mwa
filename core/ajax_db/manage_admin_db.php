...
include('../init.php');
	...
	if (isset($_POST['key'])) {
				...
				while($row = $sql->fetch_array()) {
					...
					$response .= '
						<tr>
							...
							<td  style="width:100px;"id="bot'.$row["user_id"].'">'.$row["bot"].'</td>
						...
						<tr><td colspan="7" align="right">
							...
							<input type="button" onclick="bot_approved('.$row["user_id"].', \'bot\')" value="bot" class="btn btn-success ">
							<input type="button" onclick="bot_approved('.$row["user_id"].', \'unbot\')" value="unbot" class="btn btn-success ">
							</td>
						</tr>
					';
				}
		...
		if ($_POST['key'] == 'bot') {
			$conn =$db;
			$conn->query("UPDATE users SET bot='bot' WHERE user_id='$rowID'");
			exit('The Row Has Been update!');
		}

		if ($_POST['key'] == 'unbot') {
			$conn =$db;
			$conn->query("UPDATE users SET bot='' WHERE user_id='$rowID'");
			exit('The Row Has Been update!');
		}
