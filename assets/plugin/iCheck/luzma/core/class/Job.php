...
class Job extends Home {
    ...
    public function jobsfetch()
    {
       ...
       <?php } 
        ...
        {
            $mysqli = $this->database;
            // $query= $mysqli->query("SELECT * FROM users U Left JOIN apply_job A ON A. business_id0= U. user_id LEFT JOIN jobs J ON J. job_id = A. job_id0  WHERE A. email_sent_to= '$sessions' AND A. type_of_email = 'inbox' ORDER BY A. created_on0 DESC ");
            $query= $mysqli->query("SELECT * FROM apply_job A LEFT JOIN jobs J ON J. job_id = A. job_id0 WHERE A. email_sent_to_= '$email' AND A. type_of_email = 'inbox' ORDER BY A. created_on0 DESC ");
            // var_dump($email);
        ...
        echo '
                <tr class="more" >
                    ...
                    <td class="mailbox-name inbox-view inbox-view " data-cv_id="'.$apply['cv_id'].'"  >
                    <a href="#">'.$apply['email0'].'</a></td>
        ...
        public function sentInbox($sessions)
        {
            ...
            $query= $mysqli->query("SELECT * FROM apply_job A LEFT JOIN jobs J ON J. job_id = A. job_id0 WHERE A. email_sent_for= '$sessions' AND A. type_of_email = 'sent' ORDER BY A. created_on0 DESC ");
        ...
        echo '
                <tr class="more" >
                    <td><input type="checkbox" name="a'.$apply['cv_id'].'" value="'.$apply['cv_id'].'"></td>
                    <td class="mailbox-star"><a href="#"><i class="fa fa-star text-warning"></i></a></td>
                    <td class="mailbox-name inbox-view more" data-cv_id="'.$apply['cv_id'].'"  ><a href="#">'.$apply['email0'].'</a></td>
                    <td class="mailbox-subject inbox-view " data-cv_id="'.$apply['cv_id'].'"  ><b>'.$title.' '.$message.'<b></td>
                    <td class="mailbox-attachment">'.((!empty($apply['uploadfilecv']))? '<i class="fa fa-paperclip"></i>':'' ).'</td>
                    <td class="mailbox-date">'.$this->timeAgo($apply['created_on0']).'</td>
                </tr>';
        ...
        public function trash($sessions,$email)
        {
            ...
            $query= $mysqli->query("SELECT * FROM trash T LEFT JOIN jobs J ON J. job_id = T. job_id0 WHERE T. email_sent_to_= '$email' ORDER BY T.created_on0 DESC ");
        ...
        echo '
                <tr class="more">
                    ...
                    <td class="mailbox-star"><a href="#"><i class="fa fa-star text-warning"></i></a></td>
                    <td class="mailbox-name trash-view" data-trash_id="'.$trash['trash_id'].'"  ><a href="#">'.$trash['email0'].'</a></td>
                    <td class="mailbox-subject trash-view" data-trash_id="'.$trash['trash_id'].'"><b>'.$title.' '.$message.'<b></td>
        ...
        {
            $mysqli= $this->database;
            $sql = "INSERT INTO $table (cv_id, firstname0, middlename0, lastname0, email0, address0, telephone,
            ...
            telephone, uploadfilecv, uploadfilecertificates, addition_information,cv_file_size,certificates_file_size, user_id0, job_id0, business_id0,
            ...
            $sql_ = "DELETE FROM apply_job WHERE cv_id= $id ";
            $query_= $mysqli->query($sql_);

            var_dump('ERROR: Could not able to execute'. $query_.mysqli_error($mysqli));
        ...
        public function TrashDelete($table,$id,$datetime)
        {
            ...
            $query1= "SELECT * FROM $table  WHERE trash_id= $id_ ";
    ...
    {
        $db =$this->database;
        $sql= $db->query("SELECT COUNT(*) FROM apply_job WHERE email_sent_to_= '$email' AND type_of_email = 'inbox' ");
    ...
    {
        $db =$this->database;
        $sql= $db->query("SELECT COUNT(*) FROM apply_job WHERE email_sent_for= '$user_id' AND type_of_email = 'sent' ");
        $row_unapproval = $sql->fetch_array();
        $total_unapprovalcomm= array_shift($row_unapproval);
        $array= array(0,$total_unapprovalcomm);
        $total_unapproval= array_sum($array);
    ...
    public function count_trash_job($user_id)
    {
        $db =$this->database;
        $sql= $db->query("SELECT COUNT(*) FROM trash WHERE email_sent_for= '$user_id' ");
        $row_unapproval = $sql->fetch_array();
        $total_unapprovalcomm= array_shift($row_unapproval);
        $array= array(0,$total_unapprovalcomm);
        $total_unapproval= array_sum($array);
