...
<?php }
   ...
   <?php }
    ...
    public function search($search)
    {
        $mysqli= $this->database;
        $param= '%'.$search.'%';
        $query = "SELECT user_id, username, email, career,hobbys, profile_img,followers,workname,chat,bot FROM users Where username LIKE '{$param}' OR firstname LIKE '{$param}' OR lastname LIKE '{$param}' ";
    ...
    public function bot_dark($bot,$followers)
    { ?>
        <?php return (!empty($bot) && $bot == 'bot' || $followers > 2000)?'<span><img src="'.BASE_URL_LINK.'image/img/verified-dark.png" width="15px"></span>':"" ;?>
    <?php }
...
<?php }
    ...
    public function uploadPostusermessageImage($file)
    {

        $insertValuesSQL ="";
        $targetDir = DOCUMENT_ROOT.'/uploads/postusermessageImage/';
        $allowTypes = array('jpg','png','jpeg','m4a','mp4','mp3', 'gif', 'bmp' , 'pdf' , 'doc' , 'ppt','docx', 'xlsx','xls','zip');
    ...
    { 
...

      {
        ...
        if (strpos($tweet,'www.youtube.com') !== false) {
